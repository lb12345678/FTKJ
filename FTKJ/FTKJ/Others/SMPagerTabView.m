...
- (void)buildUI {
    ...
    self.frame=CGRectMake(0, self.tabFrameY, kScreenWidth, kScreenHeight-self.tabFrameY);
    //tabView
...
- (void)selectTabWithIndex:(NSUInteger)index animate:(BOOL)isAnimate {
    for (int i=0; i<self.tabButtons.count; i++) {
        ...
        currentButton.titleLabel.font=Font(self.tabButtonFontSize);
    }
    ...
    currentButton.titleLabel.font=Font(self.tabButtonFontSize+kWidthChange(3));
