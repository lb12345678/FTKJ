...
@interface LBAlertView()<UITextFieldDelegate>
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle number:(NSInteger)number color:(UIColor *)color
{
    if (self == [super init]) {
        ...
        if (title) {
            ...
            self.titleLbl = [self GetAdaptiveLable:CGRectMake(0, 0, AlertW, 30) AndText:title andIsTitle:YES];
            ...
            self.titleLbl.frame = CGRectMake(0, 0, AlertW, 40);
            ...
            if (number == 1) {
                ...
                self.textField.frame = CGRectMake(0, CGRectGetMaxY(self.titleLbl.frame), AlertW, 40) ;
                self.textField.placeholder = @"パスワードを入力してください";
        ...
        if (number == 1) {
             self.lineView.frame = self.msgLbl?CGRectMake(0, CGRectGetMaxY(self.textField.frame), AlertW, 1):CGRectMake(0, CGRectGetMaxY(self.textField.frame), AlertW, 1);
        }else {
...
- (void)buttonEvent:(UIButton *)sender
{
    if (sender.tag == 2) {
        if (self.resultIndex) {
            if (self.number == 1) {
                self.resultIndex(sender.tag,self.textField.text);
            }else {
