...
@property (nonatomic, strong) void (^singleDoneBlock)(NSDate *date);
...
-(instancetype)initWithSingleDate:(void(^)(NSDate *date))completeBlock dismissBlock:(void(^)())dismissBlock {
    ...
    if (self) {
        ...
        if (completeBlock) {
            self.singleDoneBlock = ^(NSDate *date) {
                completeBlock(date);
            };
        }
        if (dismissBlock) {
...
-(instancetype)initWithCompleteBlock:(void(^)(NSDate *,NSDate *))completeBlock dismissBlock:(void(^)())dismissBlock{
    return [self initWithCurrentDate:nil completeBlock:completeBlock dismissBlock:dismissBlock];
}
...
- (IBAction)doneAction:(UIButton *)btn {
    ...
    if (isSingle) {
        ...
        NSDate *date = [self.scrollToDate dateWithFormatter:_dateFormatter];

        self.singleDoneBlock(date);
        [self dismiss];
        return;
    }
