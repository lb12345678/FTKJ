...
static CGFloat const kArrowImgHeight= 20;
...
- (void)rotateArrowImage {
    // 旋转箭头图片
    if (!self.isxuanzhuan) {
        _arrowImg.image=[UIImage imageNamed:@"dropdownFlag1"];
    }else{
        _arrowImg.image=[UIImage imageNamed:@"1011"];

    }
    self.isxuanzhuan=!self.isxuanzhuan;
//    _arrowImg.transform = CGAffineTransformRotate(_arrowImg.transform, M_PI/2);
}
...
- (UIImageView*)arrowImg {
    if (!_arrowImg) {
        ...
        _arrowImg.image = [UIImage imageNamed:@"10"];
